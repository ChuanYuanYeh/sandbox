...
def main():
    ...
    for sheet in args.sheets:
        df = pd.read_excel(os.path.join('data/to_be_processed', f'{args.file}.xlsx'), sheet_name=sheet, header=0)
        ...
        try:
            ...
            dct = pd.merge(sample_colors, sample_color_weights, on='recipe_id').to_dict('records')
            with open(os.path.join('data/processed', f'{sheet}.json'), 'w') as fp:
                json.dump(dct, fp)
        except KeyError:
